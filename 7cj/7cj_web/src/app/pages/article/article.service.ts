...
// 获取列表
    ...
    seePolicy(params): Observable<any> {
        return this.http.get(ROOT_URL + '/sysReadArticle', {params})
            .map(this.extractData)
            .catch(this.handleError);
    }
    // 删除政策
    delete(params): Observable<any> {
        return this.http.post(ROOT_URL + '/sys/article/delete', params)
            .map(this.extractData)
            .catch(this.handleError);
    }
    // 政策上下架
    policyState(params): Observable<any> {
        return this.http.get(ROOT_URL + '/sysCheckArticle', {params})
            .map(this.extractData)
            .catch(this.handleError);
    }
    // 标签列表
    labelList(params): Observable<any> {
        return this.http.get(ROOT_URL + '/category/getAllCategory')
            .map(this.extractData)
            .catch(this.handleError);
    }
    // 基地列表
    ...
    edit(params): Observable<any> {
        return this.http.post(ROOT_URL + '/sys/uploadArticle', params)
            .map(this.extractData)
            .catch(this.handleError);
    }
    //  政策审核
