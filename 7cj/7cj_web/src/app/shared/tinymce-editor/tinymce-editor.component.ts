...
export class TinymceEditorComponent implements OnInit, OnDestroy, AfterViewInit {
            ...
            file_browser_callback: (field_name) => {
                ...
                input.onchange = () => {
                    ...
                    reader.onload = () => {
                        ...
                        xhr.open('POST', ROOT_URL + '/front/sysUploadFile');
                        xhr.send(formData);
                    }
                }
            },
    ...
    uploadComplete(evt, field_name) {
        (<HTMLInputElement>document.getElementById(field_name)).value = IMG_URL + JSON.parse((JSON.parse(evt.target.responseText)['data']))[0];
